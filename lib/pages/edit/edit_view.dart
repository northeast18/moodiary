        ...
        children: [
          ...
          GetBuilder<EditLogic>(
            ...
            builder: (_) {
              return AdaptiveListTile(
                title: '封面图',
                subtitle: state.customCoverPath != null
                    ? const Text('自定义封面已设置 (长按清除)')
                    : (state.imageFileList.isNotEmpty ? const Text('自动提取首图为封面') : const Text('未设置')),
                trailing: state.customCoverPath != null
                    ? InkWell(
                        onLongPress: logic.removeCustomCover,
                        onTap: () => logic.pickCustomCover(context),
                        child: Container(
                          width: 40,
                          height: 40,
                          decoration: BoxDecoration(
                            ...
                            image: DecorationImage(
                              image: FileImage(File(state.customCoverPath!)),
                              fit: BoxFit.cover,
                            ),
                          ),
                        ),
                    ...
                    : IconButton.filledTonal(
                        onPressed: () async {
                          await logic.pickCustomCover(context);
                        },
