...
class DiaryLogic extends GetxController with GetTickerProviderStateMixin {
  ...
  Future<void> autoSync() async {
    final autoSyncEnabled = PrefUtil.getValue<bool>('autoSync') == true;
    final hasConnectivity = await WebDavUtil().checkConnectivity();
    logger.d('[AUTO_SYNC] autoSyncEnabled: $autoSyncEnabled, hasConnectivity: $hasConnectivity');

    if (autoSyncEnabled && hasConnectivity) {
      final diary = await IsarUtil.getAllDiaries();
      logger.d('[AUTO_SYNC] Starting sync with ${diary.length} diaries');
      ...
      logger.d('[AUTO_SYNC] Sync skipped - autoSync: $autoSyncEnabled, connectivity: $hasConnectivity');
    }
  }
