...
class _ImagesViewState extends State<ImagesView> {
  ...
  Widget build(BuildContext context) {
    if (widget.customCoverImage != null) {
      return _buildSingleImage(widget.customCoverImage!, heroPrefix);
    }
  ...
  Widget _buildSingleImage(String fileName, String heroPrefix) {
    return GestureDetector(
      onTap: () async {
        await _toPhotoView(
          [FileUtil.getRealPath(fileName.startsWith('cover-') ? 'cover' : 'image', fileName)],
          0,
          context,
          heroPrefix,
        );
      ...
      child: Hero(
        ...
        child: Image.file(
          File(fileName.startsWith('cover-') 
              ...
              : FileUtil.getRealPath('image', fileName)),
          fit: BoxFit.cover,
        ),
      ),
    );
